...
定时任务只在已有持久化登录态后执行。若查询时发现登录态失效，服务会清理旧凭证，并要求重新通过 Telegram `/start` 完成验证码登录；此时 `/powerfee/cron` 返回 `200` 与 `skipped` 状态，GitHub Actions 不会被标记为失败。
...
每次自动配置 Webhook 时会清理 Telegram pending updates，避免旧命令在服务恢复后堆积重放。
