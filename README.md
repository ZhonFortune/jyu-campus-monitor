...
嘉应学院宿舍电费余额监控服务。服务按随机 30 至 60 min 间隔查询电费余额，并在余额低于阈值时通过 Telegram 发送提醒。<strong>你可以点击下方 Deploy 字样按钮使用 Vercel 平台一键部署服务。</strong>
<br>
...
> 由于 Vercel 部署使用 Serverless API。定时查询由 `vercel.json` 中的 Cron 调用 `GET /powerfee/cron` 完成。因此，部署时需要配置 `CRON_SECRET` 环境变量以验证 Cron 调用。
...
| `CRON_SECRET` | Vercel Cron 调用 `/powerfee/cron` 的鉴权密钥 | 无 | Vercel 必填 |
